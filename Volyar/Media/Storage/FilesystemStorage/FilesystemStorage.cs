...
namespace Volyar.Media.Storage.FilesystemStorage
{
    public class FilesystemStorage : IStorage
    {
        ...
        {
            return Task.FromResult(ListFiles().Select(x => Path.GetFileName(x)));
        }
        ...
        public async Task<IEnumerable<StatusedValue<string>>> TransferAsync(IStorage destination, bool deleteSource)
        {
            ...
            foreach (var file in ListFiles())
            {
                ...
                try
                {
                    using (var fileStream = File.OpenRead(GetFullPath(file)))
                    {
        ...
        private FileSystemEnumerable<string> ListFiles()
        {
            var o = new EnumerationOptions() { RecurseSubdirectories = false, IgnoreInaccessible = true };
            var foundFiles = new FileSystemEnumerable<string>(RootDirectory, (ref FileSystemEntry entry) => entry.ToFullPath(), o)
            {
